...
public class Generator {
    ...
    public Generator(int width, int height, int RandomSeed) {
        ...
        MazePanel[NowPosX][NowPosY].setWall(0, false);
        current.push(MazePanel[NowPosX][NowPosY]);

        while (!current.isEmpty()) {
            Node status = current.peek();
            List<Node> getnb = getNeighbor(status, height, width);
            ...
            CanVisited.remove(status);
            MazePanel[status.getXY()[0]][status.getXY()[1]] = status;
    ...
    public boolean[][] getMazeResult() {
        if (MazePanel != null) {
            result = new boolean[2 * MazePanel.length + 1][2 * MazePanel[0].length + 1];
                    ...
                    for (int px = 0; px < Moving.length; px++) {
                        for (int py = 0; py < Moving[0].length; py++) {
                            if (Moving[px][py].X != 0 && Moving[px][py].Y != 0) {
                                ...
                                result[currentX][currentY] = false;
                            }
                        }
                    }
    ...
    public List<Node> getNeighbor(Node x, int height, int width) {
        ...
        for (Point xy : Shift) {
            if (currentX + xy.X >= 0 && currentX + xy.X < width
                ...
                if (!MazePanel[neighborX][neighborY].isVisted) {
                    result.add(MazePanel[neighborX][neighborY]);
                }
            }
        }
        return result;
